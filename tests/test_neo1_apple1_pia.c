...
static void test_machine_routing(void) {
    ...
    CHECK(neo1_soft65c02_mem_read(&machine, 0xD00F) == 0x21);
    CHECK(neo1_soft65c02_mem_read(&machine, 0xD014) == 0x22);
    CHECK(neo1_soft65c02_mem_read(&machine, 0xD0F1) == 0x23);
    CHECK(neo1_soft65c02_mem_read(&machine, 0xD0F4) == 0x24);

    neo1_soft65c02_mem_write(&machine, NEO1_IO_DSPCR, 0x04);
    neo1_soft65c02_mem_write(&machine, NEO1_IO_DSP_ALT, 0xD1);
    CHECK(g_display_count == 1);
    CHECK(g_display_byte == 0xD1);

    neo1_key_down(&machine, 'C');
    neo1_key_down(&machine, 'D');
    neo1_soft65c02_mem_write(&machine, NEO1_IO_KBDCR, 0x04);
    CHECK(neo1_soft65c02_mem_read(&machine, NEO1_IO_KBD) == (uint8_t)('C' | 0x80));
    CHECK(neo1_soft65c02_mem_read(&machine, NEO1_IO_KBDCR) == 0x04);

    neo1_soft65c02_mem_write(&machine, NEO1_IO_DSPCR, 0x7F);
